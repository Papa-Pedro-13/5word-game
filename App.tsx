...
function App() {
  ...
  const handleDeleteSymbol = () => {
    ...
    setStringArr(
      stringArr.map((item, index) => {
        if (index === currentRow) {
          return item.slice(0, cur - 1) + ' ' + item.slice(cur, item.length);
        }
        return item;
      }),
    );
  ...
  const checkWord = () => {
    ...
    if (currentRow === 4) {
      setModalTitle('Вы проиграли! Загаданное слово: ' + words[secretWord]);
      return;
    }
  ...
  return (
    <ScrollView style={tw`bg-gray-600`}>
      <View
        style={tw`flex flex-col gap-4 items-center justify-center relative pt-25`}>
        <TouchableOpacity
          style={tw`absolute top-8 left-8 rounded-full min-w-12.5 min-h-12.5 flex items-center justify-center  border-[2px] border-white p-1.5 hover:opacity-50`}
        ...
        <TouchableOpacity
          ...
          <Text style={{color: '#fff', fontSize: 18}}>Проверить слово</Text>
        </TouchableOpacity>
        <View>
          <Keyboard
        ...
        <View
          style={tw`
                ...
                `}>
          <View
            style={tw`flex flex-col gap-5 items-center bg-white rounded p-10`}>
